...
function solve() {
    ...
    const distanceOutput = document.querySelector('input#outputDistance');
    const convertButton = document.querySelector('input#convert');

    convertButton.addEventListener('click', handleDistanceConvert);

    function handleDistanceConvert() {
        const distance = Number(distanceInput.value);
        const fromUnit = unitsInput.value;
        const toUnit = outputUnits.value;

        distanceOutput.value = (distance * units[fromUnit]) / units[toUnit];
    };
};